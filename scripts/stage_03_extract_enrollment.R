...
source("R/stage_03_enrollment.R")
...
output_path <- args[["out-file"]] %||% stage03_default_output_path(cfg)
qc_path <- args[["qc-file"]] %||% stage03_default_qc_path(cfg)
for (path in c(output_path, qc_path)) {
  if (!grepl("^/", path)) {
    stop(
      "Stage 03 output paths must be absolute because this stage writes restricted row-level derived data: ",
      path,
      call. = FALSE
    )
  }
...
result <- extract_candidate_enrollment(
  cfg = cfg,
  candidate_path = args[["candidate-file"]],
  enrollment_paths = split_paths(args[["enrollment-files"]]),
...
cat("Stage 03 enrollment extraction complete\n")
cat("Candidate file:", result$candidate_path, "\n")
cat("Enrollment files:", length(result$enrollment_paths), "\n")
